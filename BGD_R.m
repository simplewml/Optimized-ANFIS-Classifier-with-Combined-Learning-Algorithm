function [RCEtrain,RCEvalidation,RCEtest,BCEtrain,BCEvalidation,BCEtest,BT,CT,SigmaT]=BGD_R(XTrain,yTrain,XValidation,yValidation,XTest,yTest,alpha,rr,numMFs,numIt,batchSize)
...
for it=1:numIt %numIt次训练
    ...
    if it>1 && RCEtrain(it)<RCEtrain(it-1)%如果这次的RMSE<上次RMSE，记为连续下降次数+1，否则，连续下降次数置0
        numDown=numDown+1;
    else
        numDown=0;
    end
    ...
    if it>2 && ~mod(it,2) % evaluate every two iterations
        if RCEtrain(it)<RCEtrain(it-1) && RCEtrain(it-1)>RCEtrain(it-2)
